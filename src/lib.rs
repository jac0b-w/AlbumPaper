            ...
            for (pixel_num, pixel) in block.chunks_exact_mut(3).enumerate() {
                ...
                for (i, subpix) in pixel.iter_mut().enumerate() {
                    *subpix = ((to_color[i] as f64 * scaled_dist) +
                        (from_color[i] as f64 * (1.0 - scaled_dist))) as u8;
                }
            }
        });
...
pub fn radial_gradient(
    ...
    let max_dist = distance((geometry.0/2) as i32, (geometry.1/2) as i32) - (foreground_size/2) as f64;
    ...
    background
        ...
        .for_each(|(chunk_num, block)| {
            for (pixel_num, pixel) in block.chunks_exact_mut(3).enumerate() {
                let x_dist = ((chunk_num*CHUNK_SIZE + pixel_num) as i32 % geometry.0) - (geometry.0/2);
                let y_dist = ((chunk_num*CHUNK_SIZE + pixel_num) as i32 / geometry.0) - (geometry.1/2);
